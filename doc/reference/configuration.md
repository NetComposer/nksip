...
They are defined as standard Erlang environment variables for `nksip` application, and all of them has a default value. Services cannot override them.
...
dns_cache_ttl|`integer()`|3600|DNS cache TTL (See `nksip_dns`) (secs) (Services cannot override)
local_data_path|`string()`|"log"|Path to store UUID files (Services cannot override)
...
Can also be defined as standard Erlang environment variables for `nksip` application, but Services can override most of them when starting.
...
transports|[Proto&#124;{Proto, Ip}&#124;{Proto, Ip, Port}&#124;{Proto, Ip, Port, Opts}], Proto::`nksip:protocol()`, Ip::`inet:ip_address()`&#124;`string()`&#124;`binary()`&#124;all&#124;all6, Port::`inet:port_number()`&#124;any|`[udp]`|The Service can start any number of transports. If an UDP transport is started, a TCP transport on the same IP and port will be started automatically. Use `all` to use _all_ available IPv4 addresses and `all6` for all IPv6 addresses, and `any` to use any available port
...
events|`string()`&#124;`binary()`|""|Lists the Event Packages this Service supports
