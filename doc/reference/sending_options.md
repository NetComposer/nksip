...
{subscription_state, ST}|ST::`{active, Expires}`&#124;`{pending, Expires}`&#124;`{terminated, Reason}`&#124;`{terminated, Reason, Retry}`, Expires::`undefined`&#124;integer()`, Reason::`undefined`&#124;`atom()`|Replaces Subscription-State header|To be used only in NOTIFY requests.
