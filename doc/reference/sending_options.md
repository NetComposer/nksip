...
get_request||If present, and the callback function is also present, it will be called when the request has been sent as `{req, Request, Call}`|You can use the functions in the [Request API](../api/requests.md) to extract relevant information from the request
get_meta|`[nksip_sipmsg:field()]`|Use it to select which specific fields from the final response shall be returned|Available fields are describe in [Metadata Fields](metadata.md)
