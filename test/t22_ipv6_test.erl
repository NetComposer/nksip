...
start() ->
    ...
    {ok, _} = nksip:start_link(ipv6_test_server1, #{
        sip_from => "sip:ipv6_test_server1@nksip",
        sip_local_host6 => "::1",
        plugins => [nksip_registrar],
        sip_listen => "sip:all:5060, sip:[::1]:5060"
    }),

    {ok, _} = nksip:start_link(ipv6_test_server2, #{
        sip_from => "sip:ipv6_test_server2@nksip",
        sip_local_host => "127.0.0.1",
        sip_local_host6 => "::1",
        sip_listen => "sip:all:5061, sip:[::1]:5061"
    }),

    {ok, _} = nksip:start_link(ipv6_test_client1, #{
        sip_from => "sip:ipv6_test_client1@nksip",
        sip_listen => "sip:[::1]:5070"
    }),
    
    {ok, _} = nksip:start_link(ipv6_test_client2, #{
        sip_from => "sip:ipv6_test_client2@nksip",
        sip_listen => "sip:[::1]:5071"
    }),

    {ok, _} = nksip:start_link(ipv6_test_client3, #{
        sip_from => "sip:ipv6_test_client3@nksip",
        sip_local_host => "127.0.0.1",
...
stop() ->
    ok = nksip:stop(ipv6_test_server1),
...
basic() ->
    ...
    Fun1 = fun
        ({req, #sipmsg{contacts=[Contact]}, _Call}) ->
            #uri{user=(<<"ipv6_test_client1">>), domain=MainIp, port=5070} = Contact,
            Self ! {Ref, ok_1};
    ...
    {async, _} = nksip_uac:options(ipv6_test_client1, RUri1, Opts1),
    ?RECV({Ref, ok_1}),
    ?RECV({Ref, {ok_2, MainIp}}),
    
    Fun2 = fun
        ({req, #sipmsg{contacts=[Contact]}, _Call}) ->
            #uri{user=(<<"ipv6_test_client1">>), domain=MainIp, port=5070, opts=COpts2} = Contact,
    ...
    {async, _} = nksip_uac:options(ipv6_test_client1, RUri2, Opts2),
    ?RECV({Ref, ok_3}),
    ?RECV({Ref, ok_4}),
    ok.
...
invite() ->
    ...
    {ok, 200, [{dialog, DialogId1}]} = nksip_uac:invite(ipv6_test_client1, RUri, Hds),
    ok = nksip_uac:ack(DialogId1, []),
    ok = tests_util:wait(Ref, [{ipv6_test_client2, ack}]),
    ...
    DialogId2 = nksip_dialog_lib:remote_id(DialogId1, ipv6_test_client2),
    {ok, 200, []} = nksip_uac:options(DialogId2, []),
    {ok, 200, [{dialog, DialogId2}]} = nksip_uac:invite(DialogId2, Hds),
    ok = nksip_uac:ack(DialogId2, []),
    ok = tests_util:wait(Ref, [{ipv6_test_client1, ack}]),
    {ok, 200, []} = nksip_uac:bye(DialogId2, []),
    ok = tests_util:wait(Ref, [{ipv6_test_client1, bye}]),
    ok.
...
proxy() ->
    ...
    {ok, 200, []} = nksip_uac:register(ipv6_test_client1, S1Uri, [unregister_all]),
    {ok, 200, []} = nksip_uac:register(ipv6_test_client2, S1Uri, [unregister_all]),
    
    % Avoid outbound support
    {ok, 200, []} = nksip_uac:register(ipv6_test_client1, S1Uri, [contact, {supported, ""}]),
    {ok, 200, []} = nksip_uac:register(ipv6_test_client2, S1Uri, [contact, {supported, ""}]),

    %% ipv6_test_client1 will send an INVITE to ipv6_test_client2
    ...
    %% Server2 routes to ipv6_test_client2 (stateful, record_route)
    Route = {route, "<sip:[::1];lr>"},
    {ok, 200, [{dialog, DialogId1}, {<<"x-nk-id">>, [<<"ipv6_test_client2,ipv6_test_server2,ipv6_test_server1">>]}]} =
        nksip_uac:invite(ipv6_test_client1, "sip:ipv6_test_client2@nksip", [Route, {get_meta, [<<"x-nk-id">>]},
                                                  {supported, ""}|Hds]),
    ...
    AckFun = fun({req, AckReq, _Call}) ->
        [#uri{domain=(<<"[::1]">>), port=5061, opts=AckOpts}] = 
            nksip_sipmsg:get_meta(routes, AckReq),
...
bridge_4_6() ->
    ...
    {ok, 200, []} = nksip_uac:register(ipv6_test_client1, "sip:[::1]", [unregister_all]),
    {ok, 200, []} = nksip_uac:register(ipv6_test_client3, "sip:127.0.0.1", [unregister_all]),
    
    % Avoid outbound support
    {ok, 200, []} = nksip_uac:register(ipv6_test_client1, "sip:[::1]", [contact, {supported, ""}]),
    {ok, 200, []} = nksip_uac:register(ipv6_test_client3, "sip:127.0.0.1", [contact, {supported, ""}]),

    %% ipv6_test_client1 will send an INVITE to ipv6_test_client3
    ...
    %% Server2 routes to ipv6_test_client3 (stateful, record_route, IPv4)
    Route1 = {route, "<sip:[::1];lr>"},
    Fields1 = {get_meta, [<<"x-nk-id">>, contacts]},
    {ok, 200, Values1} = nksip_uac:invite(ipv6_test_client1, "sip:ipv6_test_client3@nksip",
                                [Route1, Hd, Fields1, {supported, ""}]),
    %% ipv6_test_client3 has generated a IPv4 Contact
    [
        {dialog, DialogId1}, 
        {<<"x-nk-id">>, [<<"ipv6_test_client3,ipv6_test_server2,ipv6_test_server1">>]},
    ...
    FunAck = fun({req, ReqAck1, _Call}) ->
        [#uri{domain=(<<"[::1]">>), port=5061, opts=AckOpts}] = 
            nksip_sipmsg:get_meta(routes, ReqAck1),
        true = lists:member(<<"lr">>, AckOpts),
        true = lists:member({<<"transport">>, <<"tcp">>}, AckOpts),
        #uri{domain=(<<"127.0.0.1">>)} = nksip_sipmsg:get_meta(ruri, ReqAck1),
        Self ! {Ref, ok_1}
    end,
    async  = nksip_uac:ack(DialogId1, [async, {callback, FunAck}]),
    ?RECV({Ref, ok_1}),
   
    DialogId3 = nksip_dialog_lib:remote_id(DialogId1, ipv6_test_client3),
