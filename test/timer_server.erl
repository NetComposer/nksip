%% -------------------------------------------------------------------
%%
%% timer_server: Server Callback module for timer test
%%
...
route(_, _, _, Domain, _, #state{id={timer, p1}}=State) ->
    OptsA = [{headers, [{"Nk-Id", "p1"}]}],
    OptsB = [{route, "<sip:127.0.0.1:5071;lr;transport=tls>"}, make_timer|OptsA],
    case Domain of 
...
% It sends everything to P2, inserting Path header
route(_, _, _, Domain, _, #state{id={timer, p2}}=State) ->
...
route(_, _, _, Domain, _, #state{id={timer, p3}}=State) ->
    OptsA = [{headers, [{"Nk-Id", "p3"}]}],
    OptsB = [{route, "<sip:127.0.0.1:5090;lr>"}, make_timer|OptsA],
    case Domain of 
        <<"nksip">> -> {reply, {proxy, ruri, OptsB}, State};
        _ -> {reply, {proxy, ruri, OptsA}, State}
    end;
...
route(_ReqId, Scheme, User, Domain, _From, #state{id={timer, registrar}}=State) ->
    case Domain of
        <<"nksip">> when User == <<>> ->
            {reply, process, State};
        <<"nksip">> ->
            case nksip_registrar:find({timer, registrar}, Scheme, User, Domain) of
