...
start() ->
    ...
    {ok, _} = nksip:start_link(event_test_client1, #{
        sip_from => "sip:event_test_client1@nksip",
        sip_local_host => "localhost",
        sip_event_expires_offset => 0,
        sip_events => "myevent1,myevent2,myevent3"
    }),
    
    {ok, _} = nksip:start_link(event_test_client2, #{
        sip_from => "sip:event_test_client2@nksip",
        sip_no_100 => true,
...
basic() ->
    ...
    {ok, 489, []} = 
        nksip_uac:subscribe(event_test_client1, SipC2, [{event, "myevent1;id=a"}, CB, get_request]),
    ...
    {ok, 200, [{subscription, Subs1A}, {"event", [<<"myevent4;id=4;o=2">>]}]} =
        nksip_uac:subscribe(event_test_client1, SipC2,
            [{event, "myevent4;id=4;o=2"}, {expires, 1}, RepHd, {get_meta, ["event"]}]),

    {ok, Dialog1A} = nksip_dialog:get_handle(Subs1A),
    Dialog1B = nksip_dialog_lib:remote_id(Dialog1A, event_test_client2),
    Subs1B = nksip_subscription_lib:remote_id(Subs1A, event_test_client2),
    {ok, [
        {status, init},
    ...
    ]} = nksip_subscription:get_metas([status, event, class, answered, expires], Subs1A),
    ...
    {ok, [
        ...
        {answered, undefined},
        {expires, 1} 
    ]} = nksip_subscription:get_metas([status, event, class, answered, expires], Subs1B),

    {ok, [
        {invite_status, undefined},
        {subscriptions, [Subs1A]}
    ]} = nksip_dialog:get_metas([invite_status, subscriptions], Dialog1A),
    {ok, [
        {invite_status, undefined},
        {subscriptions, [Subs1B]}
    ]} = nksip_dialog:get_metas([invite_status, subscriptions], Dialog1B),
    ...
    {error, _} = nksip_subscription:get_meta(status, Subs1A),
    {error, _} = nksip_subscription:get_meta(status, Subs1B),


    {ok, 200, [{subscription, Subs2A}]} = 
        nksip_uac:subscribe(event_test_client1, SipC2, [{event, "myevent4;id=4;o=2"}, {expires, 2}, RepHd]),
 
    Subs2B = nksip_subscription_lib:remote_id(Subs2A, event_test_client2),
    ok = tests_util:wait(Ref, [{subs, init, Subs2B}]),

    {ok, Dialog2A} = nksip_dialog:get_handle(Subs2A),
    tests_util:update_ref(event_test_client1, Ref, Dialog2A),
    ...
    ok = tests_util:wait(Ref, [
        {subs, pending, Subs2B}, 
        {subs, pending, Subs2A},
        {event_test_client1, {notify, <<"notify1">>}},
    ...
    ok = tests_util:wait(Ref, [
        {subs, active, Subs2B}, 
        {subs, active, Subs2A},
        {event_test_client1, {notify, <<"notify2">>}},
        {subs, middle_timer, Subs2A},         % event_test_client1 has re-scheduled timers
        {subs, middle_timer, Subs2B}  
    ]),
...
refresh() ->
    ...
    {ok, 200, [{subscription, Subs1A}]} = 
        nksip_uac:subscribe(event_test_client1, SipC2, [{event, "myevent4"}, {expires, 5}, Hd1, Hd2]),
    Subs1B = nksip_subscription_lib:remote_id(Subs1A, event_test_client2),
    {ok, 200, []} = nksip_uac:notify(Subs1B, []),

    % 2xx response to subscribe has changed timeout to 2 secs
    {ok, 2} = nksip_subscription:get_meta(expires, Subs1A),
    {ok, 2} = nksip_subscription:get_meta(expires, Subs1B),
    ok = tests_util:wait(Ref, [
        {subs, init, Subs1B},
        {subs, active, Subs1B},
        {subs, middle_timer, Subs1B}
    ...
    {ok, 20} = nksip_subscription:get_meta(expires, Subs1A),
    {ok, 20} = nksip_subscription:get_meta(expires, Subs1B),
...
dialog() ->
    ...
    {ok, 200, [{subscription, Subs1A}, {dialog_handle, DialogA}]} = 
        nksip_uac:subscribe(event_test_client1, SipC2, [{event, "myevent4;id=1"}, {expires, 2},
                                             {contact, "sip:a@127.0.0.1"}, {get_meta, [dialog_handle]}]),
    Subs1B = nksip_subscription_lib:remote_id(Subs1A, event_test_client2),
    ...
    DialogB = nksip_dialog_lib:remote_id(DialogA, event_test_client2),
    ...
    ]} = nksip_dialog:get_metas([raw_local_target, raw_remote_target, raw_route_set], DialogB),

    % Now we have a dialog with 2 subscriptions
    {ok, [Subs1A, Subs2A]} = nksip_dialog:get_meta(subscriptions, DialogA),
    ...
    Subs3A = nksip_subscription_lib:remote_id(Subs3B, event_test_client1),
    {ok, 200, []} = nksip_uac:notify(Subs3A, []),

    % Now we have a dialog with 3 subscriptions and a INVITE
    {ok, [Subs1A, Subs2A, Subs3A]} = nksip_dialog:get_meta(subscriptions, DialogA),
    {ok, [Subs1B, Subs2B, Subs3B]} = nksip_dialog:get_meta(subscriptions, DialogB),
    {ok, confirmed} = nksip_dialog:get_meta(invite_status, DialogA),
    {ok, confirmed} = nksip_dialog:get_meta(invite_status, DialogB),

    timer:sleep(2000),

    % Now the subscriptions has timeout, we have only the INVITE
    {ok, []} = nksip_dialog:get_meta(subscriptions, DialogA),
    {ok, []} = nksip_dialog:get_meta(subscriptions, DialogB),
    {ok, confirmed} = nksip_dialog:get_meta(invite_status, DialogA),
    {ok, confirmed} = nksip_dialog:get_meta(invite_status, DialogB),

    {ok, 200, []} = nksip_uac:bye(DialogB, []),
    {error, _} = nksip_dialog:get_meta(invite_status, DialogA),
    {error, _} = nksip_dialog:get_meta(invite_status, DialogB),
    ok.
...
out_or_order() ->
    ...
    {async, _} = 
        nksip_uac:subscribe(event_test_client1, SipC2, [{event, "myevent4"}, CB, async,
    ...
    % Generate a NOTIFY similar to what event_test_client2 would send after 200, simulating
    % coming to event_test_client1 before the 200 response to SUBSCRIBE
...
fork() ->
    ...
    SubsB = nksip_subscription_lib:remote_id(SubsA, event_test_client2),
    ...
    [D1, D2, D3, D4] = nksip_dialog:get_all(event_test_client1, CallId),
    {ok, [_]} = nksip_dialog:get_meta(subscriptions, D1),
    {ok, [_]} = nksip_dialog:get_meta(subscriptions, D2),
    {ok, [_]} = nksip_dialog:get_meta(subscriptions, D3),
    {ok, [_]} = nksip_dialog:get_meta(subscriptions, D4),
