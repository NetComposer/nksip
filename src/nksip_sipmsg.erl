...
-type field() ::  
    id | internal_id | app_id | app_name | dialog_id | subscription_id |
    ...
    expires | event | realms | rseq_num | rack | 
...
-spec meta(field()|string()|binary(), nksip:request()|nksip:response()) ->
    term().

meta(Name, SipMsg) when is_list(Name); is_binary(Name) ->
    header(Name, SipMsg);
