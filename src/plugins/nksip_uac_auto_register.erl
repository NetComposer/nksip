...
start_register(PkgId, RegId, Uri, Opts) when is_list(Opts) ->
    try
        ...
        case nksip_call_uac_make:make(PkgId, 'REGISTER', Uri, CallId, Opts) of
            {ok, _, _} ->
                ok;
            {error, MakeError} ->
                throw(MakeError)
...
stop_register(PkgId, RegId) ->
     nkserver_srv:call(PkgId, {nksip_uac_auto_register_stop_reg, RegId}).
...
get_registers(PkgId) ->
     nkserver_srv:call(PkgId, nksip_uac_auto_register_get_regs).
...
start_ping(PkgId, PingId, Uri, Opts) when is_list(Opts) ->
    try
        ...
        CallId = nklib_util:luid(),
        case nksip_call_uac_make:make(PkgId, 'OPTIONS', Uri, CallId, Opts) of
            {ok, _, _} ->
                ok;
            {error, MakeError} ->
                throw(MakeError)
...
get_pings(PkgId) ->
     nkserver_srv:call(PkgId, nksip_uac_auto_register_get_pings).
