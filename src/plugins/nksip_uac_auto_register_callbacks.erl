...
-export([nksip_uac_auto_register_send_reg/3,
         ...
         nksip_uac_auto_register_send_ping/2,
...
service_handle_call({nksip_uac_auto_register_stop_reg, RegId}, 
    ...
    case lists:keytake(RegId, #sipreg.id, Regs) of
        {value, Reg, Regs1} -> 
            ...
            {ok, SvcState2} = 
                ?CALL_SRV(SrvId, nksip_uac_auto_register_send_unreg, [Reg, false, SvcState1]),
            {reply, ok, SvcState2};
        false -> 
...
service_handle_cast({nksip_uac_auto_register_reg_reply, RegId, Code, Meta}, 
    ...
    case lists:keytake(RegId, #sipreg.id, Regs) of
        {value, #sipreg{ok=OldOK}=Reg, Regs1} ->
            {ok, Reg1, SvcState1} = 
                ?CALL_SRV(SrvId, nksip_uac_auto_register_upd_reg, [Reg, Code, Meta, SvcState]),
            #sipreg{ok=Ok} = Reg1,
            case Ok of
                OldOK -> 
                    ok;
    ...
    case lists:keytake(PingId, #sipreg.id, Pings) of
        {value, #sipreg{ok=OldOK}=Ping, Pings1} ->
            {ok, #sipreg{ok=OK}=Ping1, SvcState1} = 
                ?CALL_SRV(SrvId, nksip_uac_auto_register_upd_ping, [Ping, Code, Meta, SvcState]),
            case OK of
                OldOK -> 
                    ok;
                _ -> 
...
nksip_uac_auto_register_send_reg(Reg, Sync, #{id:=SrvId}=SvcState)->
    #sipreg{id=RegId, ruri=RUri, opts=Opts, cseq=CSeq} = Reg,    
    Opts1 = [contact, {cseq_num, CSeq}, {meta, [cseq_num, retry_after]}|Opts],
    Self = self(),
    Fun = fun() ->
        {Code, Meta} = case nksip_uac:register(SrvId, RUri, Opts1) of
            {ok, Code0, Meta0} ->
                {Code0, Meta0};
            _ ->
                {500, [{cseq_num, CSeq}]}
...
nksip_uac_auto_register_send_unreg(Reg, Sync, #{id:=SrvId}=SvcState)->
...
nksip_uac_auto_register_upd_reg(Reg, Code, _Meta, SvcState) when Code<200 ->
    {ok, Reg, SvcState};
...
%% @private
-spec nksip_uac_auto_register_send_ping(#sipreg{}, nkservice_server:sub_state()) ->
    {ok, #sipreg{}, nkservice_server:sub_state()}.

nksip_uac_auto_register_send_ping(Ping, #{id:=SrvId}=SvcState)->
    #sipreg{id=PingId, ruri=RUri, opts=Opts, cseq=CSeq} = Ping,
    Opts1 = [{cseq_num, CSeq}, {meta, [cseq_num, retry_after]} | Opts],
    Self = self(),
    Fun = fun() ->
        {Code, Meta} = case nksip_uac:options(SrvId, RUri, Opts1) of
            {ok, Code0, Meta0} ->
                {Code0, Meta0};
            _ ->
                {500, [{cseq_num, CSeq}]}
...
nksip_uac_auto_register_upd_ping(Ping, Code, _Meta, SvcState) when Code<200 ->
    {ok, Ping, SvcState};
...
check_pings(Now, [#sipreg{next=Next}=Ping|Rest], Acc, #{id:=SrvId}=SvcState) ->
    case is_integer(Next) andalso Now>=Next of 
        true -> 
            {ok, Ping1, SvcState1} = 
                ?CALL_SRV(SrvId, nksip_uac_auto_register_send_ping, [Ping, SvcState]),
...
check_registers(Now, [#sipreg{next=Next}=Reg|Rest], Acc, #{id:=SrvId}=SvcState) ->
    case Now>=Next of
        true -> 
            {ok, Reg1, SvcState1} = 
                ?CALL_SRV(SrvId, nksip_uac_auto_register_send_reg, [Reg, false, SvcState]),
