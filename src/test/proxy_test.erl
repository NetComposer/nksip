...
invalid() ->
    ...
    {ok, C1Id} = nkservice_srv:get_srv_id(C1),
    {ok, Req4, Opts4} = nksip_call_uac_make:make(C1Id, <<"Sip">>, 'REGISTER', "sip:any", []),
    {ok, 483, _} = nksip_call:send(Req4#sipmsg{forwards=0}, Opts4),

    % Force Forwards=0 using OPTIONS. Server will reply
    {ok, Req5, Opts5} = nksip_call_uac_make:make(C1Id, <<"Sip">>, 'OPTIONS', "sip:any", []),
...
opts() ->
    ...
    [Uri2] = nksip_registrar:find(server1, sip, <<"client2">>, <<"nksip">>),
    Opts4 = [{route, 
...
sip_route(Scheme, User, Domain, Req, _Call) ->
    ...
    case nksip_request:srv_id(Req) of
        {ok, SrvId} when SrvId==server1; SrvId==server2 ->
            Opts = [
                {insert, "x-nk-id", SrvId},
            ...
            case lists:member(Domain, Domains) of
                ...
                true when User =:= <<"client2_op">>, Domain =:= <<"nksip">> ->
                    UriList = nksip_registrar:find(SrvId, sip, <<"client2">>, Domain),
                ...
                true when Domain =:= <<"nksip">>; Domain =:= <<"nksip2">> ->
                    case nksip_registrar:find(SrvId, Scheme, User, Domain) of
                        [] -> 
                            % ?P("FIND ~p: []", [{SrvId, Scheme, User, Domain}]),
...
sip_invite(Req, _Call) ->
    ...
    Hds = [
        ...
        {add, "x-nk-id", nklib_util:bjoin([SrvId|Ids])}
    ],
