    ...
    {Req1, Opts1} = case Term of
        ...
        {route, Route} ->
            case nksip_parse:uris(Route) of
                ...
                Uris -> {Req#sipmsg{routes=Uris}, Opts}
            end;
        {contact, Contact} ->
        ...
        {Name, Value} ->
            case nksip_parse_header:header_name(Name) of
                same -> throw({invalid, Name});
                Name -> Req#sipmsg{headers=Req#sipmsg.headers++[{Name, Value}]}
            end;
        _ ->
            throw({invalid, Term})
    end,
...
write_header(#sipmsg{headers=Headers}=Req, Name, Value) ->
    Req#sipmsg{headers=nksip_lib:store_value(Name, Value, Headers)}.
