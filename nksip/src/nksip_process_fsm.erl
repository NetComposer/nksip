...
init([AppId, CallId, Method]) ->
    ...
    case nksip_sipapp_srv:get_module(AppId) of
        {Module, Pid} ->
            ...
            AppOpts1 = nksip_lib:extract(AppOpts, [local_host, registrar, no_100]),
            SD = #state{
                app_id = AppId, 
                call_id = CallId, 
                module = Module, 
...
init(timeout, #state{app_id=AppId, call_id=CallId}=SD) ->
    ?error(AppId, CallId, "Process FSM init timeout", []),
    {stop, init_timeout, SD};



handle_event({insert, #raw_sipmsg{class={req, Method, _Binary}}=RawMsg}, StateName, SD) 
             when StateName=:=init ->
    case nksip_parse:raw_sipmsg(RawMsg) of
        error ->
            #raw_sipmsg{transport=Transport} = RawMsg,
            ?notice(AppId, "could not process ~p packet: ~s", 
                    [Transport#transport.proto, Binary]),
                {next_state, StateName, SD};
        Req1 ->
            % Check RETRANS, ACK
            case nksip_uas_lib:preprocess(Req1) of
                {ok, Req2} -> 
                    Timer = case Method of
                        'INVITE' -> gen_fsm:start_timer(?INVITE_TIMEOUT, invite_timeout);
                        _ -> gen_fsm:start_timer(?NOINVITE_TIMEOUT, noinvite_timeout)
                    end,
                    uas_authorize(launch, SD#state{current=Req2, timeout_timer=Timer});
                ignore -> 
                    % It is an own-generated ACK without transaction
                    {noreply, StateName, SD}
            end
    end;



uas_authorize(launch, #state{current=Req}=SD) ->
...
%% @private
-spec uas_authorize(timeout | {reply, term()}, #state{}) -> 
    fsm_out().

uas_authorize({timeout, _, authorize}, SD) ->
    next(core_error, SD);

uas_authorize({timeout, _, timer_100}, SD) ->
    send_reply(100, SD),
    {next_state, authorize, SD};

uas_authorize({reply, Reply}, #state{req=#sipmsg{method=Method, to_tag=ToTag}=Req}=SD) ->
    nksip_trace:insert(Req, authorize_reply),
    case Reply of
        ok -> route(launch, SD);
        true -> route(launch, SD);
        false -> next(forbidden, SD);
        authenticate -> next(authenticate, SD);
        {authenticate, Realm} -> next({authenticate, Realm}, SD);
        proxy_authenticate -> next(proxy_authenticate, SD);
        {proxy_authenticate, Realm} -> next({proxy_authenticate, Realm}, SD);
        Other -> next(Other, SD)
    end.


%% @private
-spec uas_route(launch | timeout | {reply, term()} | 
...
uas_route(launch, #state{req=#sipmsg{ruri=RUri}=Req}=SD) ->
    nksip_trace:insert(Req, route),
    corecall(route, [RUri#uri.scheme, RUri#uri.user, RUri#uri.domain], SD),
    start_timer(route, SD);

uas_route({timeout, _, route}, SD) ->
    stop(core_error, SD);

uas_route({timeout, _, timer_100}, SD) ->
    send_reply(100, SD),
    {next_state, route, SD};

uas_route({reply, Reply}, #state{req=Req}=SD) ->
        ...
        {process, _} when Method=/='CANCEL', Method=/='ACK' ->
            case nksip_parse:header_tokens(<<"Require">>, Req) of
                ...
                Requires -> 
                    ...
                    next({bad_extension,  RequiresTxt}, SD)
            end;
        _ ->
            uas_route(Route, SD)
    end;

uas_route({response, Reply, Opts}, #state{req=#sipmsg{opts=ReqOpts}=Req}=SD) ->
    % Detect stateless and stop the process if no more requests
    next(Reply, SD#state{current=Req#sipmsg{opts=Opts++ReqOpts}});

uas_route({process, Opts}, #state{req=Req}=SD) ->
    #sipmsg{method=Method, headers=Headers, to_tag=ToTag, opts=ReqOpts} = Req,
    ...
    uas_process(launch, SD#state{req=Req1});

% We want to proxy the request
uas_route({proxy, UriList, Opts}, #state{req=Req}=SD) ->
...
route({strict_proxy, Opts}, #state{req=Req}=SD) ->
    ...
    case Routes of
        ...
        _ ->
            next({internal_error, <<"Invalid Srict Routing">>}, SD)
    end.
